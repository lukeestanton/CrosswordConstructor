...
test("revealer placement pins the word to the last across slot", async ({
  page,
}) => {
  // DICT plus a second fill where BAT is the BOTTOM row: WEN/ERE/BAT over
  // WEB/ERA/NET. Unconstrained, BAT can prove in row 0 or row 2; center is
  // a contradiction either way (no word has B as its middle letter).
  const REVEALER_DICT = DICT + "WEN;60\nERE;60\nWEB;60\nERA;60\nNET;60\n";
  let createdPayload: string | null = null;
  await page.route("**/api/wordlist", (route) =>
    route.fulfill({ contentType: "text/plain", body: REVEALER_DICT }),
  );
  await page.route("**/api/layouts**", (route) =>
    route.fulfill({
      json: {
  ...
  // Center mode pins BAT to the middle row — a contradiction in this dict,
  // so the layout drops instead of falling back to rows 0/2.
  await page.getByRole("radio", { name: "center row" }).click();
  await expect(statusText).toHaveText("1 published layout match", {
    timeout: 20_000,
  });
  await expect(rows).toHaveCount(0);

  // Last mode: only the bottom row sits strictly below the middle row.
  await page.getByRole("radio", { name: "last across" }).click();
  const row = rows.first().locator("button");
  await expect(row.getByText("fill proven")).toBeVisible({ timeout: 20_000 });
  await row.click();
  await expect(
  ...
  const lockedIndexes = state.cells
    .map((c: { locked?: boolean }, i: number) => (c.locked ? i : -1))
    .filter((i: number) => i >= 0);
  expect(lockedIndexes).toEqual([6, 7, 8]);
  // The cursor opens on the revealer's slot.
  expect(state.cursor).toEqual({ r: 2, c: 0, orient: "across" });
});
